...
enum rex_parse_token_e
{
    ...
    REX_PARSE_TOKEN_CHARSET_STR = 0x80,
...
{
        REX_EMIT_TOKEN_CHARSET_STR = REX_PARSE_TOKEN_CHARSET_STR,
...
)
{
    ...
    {
        /* TODO: do the swap one at a time */
        return 1;
    }
    return 0;
}
...
)
{
    ...
    for (i =0; i < ast_sz; i++)
    {
        ...
        default:
            if (!leaves) return 0;
            j = rex_parse_charset_str(ast+i, ast_sz, NULL, NULL);
            if (!j) return 0;
            tree_sz += j + 1;
            leaves--;
